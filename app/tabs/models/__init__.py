"""Tab Models — extraída de streamlit_app.py sin cambiar la lógica (fase 1).

Se extrae el módulo ENTERO. render(time_series_analysis) re-lee recovery fresco
de st.session_state (aislado de otras tabs) y prepara df_model. Los imports
locales dentro de las ramas de modelos se conservan tal cual. En fases siguientes
se subdivide en regression/ (ols, linear, nonlinear, ensemble), classification y
unsupervised.
"""
...
def render(time_series_analysis):
    ...
    if types == "Regression":
        ...
        if models == "OLS diagnosis":  # Linear Regression Selected
            ...
            if (st.session_state.model_frozen is None) and (n < 300):
                ...
                with st.expander("ℹ️ Model Summary", expanded=False):
                    ...
                    with c2:
                        ...
                        ax.axvline(
                            x=80, color="grey", linestyle=":", label="Good Quality Threshold"
                        )
                        ax.axvline(
                        ...
                        vif_data_pol["VIF"] = [
                            variance_inflation_factor(X_pol.values, i)
                    ...
                    with c1:
                        st.metric(
                            ...
                            delta_color="green"
                            if r2_test_linear_pol > r2_train_linear_pol
                            else "red",
                        )
                    with c2:
                        st.metric(
                ...
                with st.expander("📊 Residuals Analysis", expanded=False):
                    ...
                    if residuals_analysis == "On Train Set":
                        ...
                        residuals_train_df = df_model[
                            df_model.index.isin(train_lin_pol.index)
                        ].copy()
                        ...
                        bp_test = het_breuschpagan(
                            model_linear_pol.resid, model_linear_pol.model.exog
                        )
                        bp_labels = [
            ...
            # ------------------------------ DEPLOYMENT & MONITORING -----------------------------
                ...
                if st.session_state.freeze_predictors != predictors:
                    st.error(
                        "Predictors changed after model freeze. Please refresh/reset the model."
                    )
                    st.stop()
                ...
                if len(live_df) >= 5:
                    ...
                    rmse_live = np.sqrt(
                        mean_squared_error(live_df["Score"], live_df["yhat_frozen"])
                    )
            ...
            rmse_train_linear_pol = np.sqrt(
                mean_squared_error(train_lin["Score"], y_pred_train_best)
            )
                ...
                def metrics_lcv(df, x_train, x_test, y_train, y_test, model=best_linear):
                    ...
                    for ts in train_sizes:
                        ...
                        train_rmse_list.append(
                            np.sqrt(
                                mean_squared_error(y_train.iloc[:ts], m.predict(x_train.iloc[:ts]))
                            )
                        )
                    ...
                    ax[0].plot(
                        train_sizes, test_mse_mean, label="Test RMSE (holdout)", color="orange"
                    )
                    ax[0].set_title(
        ...
        # ---------------------------------- NON LINEAR MODELS ----------------------------------
            ...
            mae_train_nonlinear = mean_absolute_error(
                train_lin["Score"], y_pred_train_best_nonlinear
            )
            ...
            with st.expander("📈 Performance Metrics: Train vs Test", expanded=True):
                ...
                with c4:
                    st.metric(
                        ...
                        delta_color="red"
                        if rmse_test_nonlinear > rmse_train_nonlinear
                        else "green",
                        help="Root Mean Squared Error (RMSE): lower values indicate better fit, in original units.",
                    )
                with c5:
            ...
            with st.expander("📊 Explanatory Power of Predictors", expanded=False):
                ...
                with col2:
                    ...
                    shap.plots.waterfall(
                        shap_values_non_linear[sample_ind], max_display=14, show=False
                    )
                    st.pyplot(fig)
            ...
            with st.expander("📊 Explanatory Power of Predictors", expanded=False):
                ...
                with col2:
                    ...
                    fig, ax = plt.subplots(figsize=(8, 5))
                    shap.plots.waterfall(
                        shap_values_ensemble[sample_ind], max_display=14, show=False
                    )
                    st.pyplot(fig)
